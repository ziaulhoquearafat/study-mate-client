import { FaLaptopCode, FaLightbulb, FaRocket, FaSearch } from "react-icons/fa";

const HowItWorks = () => {
  return (
    <section className="py-20 bg-[#e9f9f8] dark:bg-gray-950">
      {/* Heading */}
      <div className="w-11/12 mx-auto text-center max-w-6xl">
        <h2 className="font-montserrat text-4xl mb-10 text-center font-bold text-[#05305a] dark:text-white">
          How It Works
        </h2>
        <p className="text-gray-600 dark:text-gray-400 text-lg mb-12 max-w-3xl mx-auto ">
          A simple 4-step process to start your journey — learn, practice, and
          grow as a skilled web developer.
        </p>
      </div>

      {/* Steps Grid */}
      <div className="w-11/12 mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-10">
        <div className="bg-white dark:bg-gray-800 flex flex-col justify-center items-center space-y-2 p-8 rounded-2xl shadow-md text-center">
          <FaLightbulb className="w-10 h-10 text-[#05305a] dark:text-amber-500" />
          <h3 className="text-lg font-semibold font-montserrat dark:text-white">
            Step 1: Sign Up
          </h3>
          <p className="text-[#555] dark:text-gray-400">
            Create your account to access the platform.
          </p>
        </div>
        <div className="bg-white dark:bg-gray-800 flex flex-col justify-center items-center space-y-2 p-8 rounded-2xl shadow-md text-center">
          <FaLaptopCode className="w-10 h-10 text-[#05305a] dark:text-amber-500" />
          <h3 className="text-lg font-semibold font-montserrat dark:text-white">
            Step 2: Create Your Profile
          </h3>
          <p className="text-[#555] dark:text-gray-400">
            Add your subjects, skills and availability.
          </p>
        </div>
        <div className="bg-white dark:bg-gray-800 flex flex-col justify-center items-center space-y-2 p-8 rounded-2xl shadow-md text-center">
          <FaSearch className="w-10 h-10 text-[#05305a] dark:text-amber-500" />
          <h3 className="text-lg font-semibold font-montserrat dark:text-white">
            Step 3: Find Study Partners
          </h3>
          <p className="text-[#555] dark:text-gray-400">
            Search and filter partners based on subject and location.
          </p>
        </div>
        <div className="bg-white dark:bg-gray-800 flex flex-col justify-center items-center space-y-2 p-8 rounded-2xl shadow-md text-center">
          <FaRocket className="w-10 h-10 text-[#05305a] dark:text-amber-500" />
          <h3 className="text-lg font-semibold font-montserrat dark:text-white">
            Step 4: Connect & Learn
          </h3>
          <p className="text-[#555] dark:text-gray-400">
            Send request or message to your partner and start learning.
          </p>
        </div>
      </div>
    </section>
  );
};

export default HowItWorks;
